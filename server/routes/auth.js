...
import { generateSixDigitCode, hashCode, compareCode } from "../utils/security.js";
...
router.post("/signup", authLimiter, async (req, res) => {
  try {
    ...
    await sendOtpEmail(email, "Verify your Nexus account", code);
...
/* =========================
   SIGNUP VERIFY
   Creates account only after OTP is correct
========================= */
router.post("/signup/verify", otpLimiter, otpValidation, validate, async (req, res) => {
  try {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await sendOtpEmail(email, "Nexus RMS password reset code", code);
